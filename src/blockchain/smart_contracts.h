...
bool ycsb_get(const RepeatedPtrField<string> &keys, Endorsement *endorsement= nullptr, long long last_block_id=-1);
...
string kv_get(const string &key, Endorsement *endorsement = nullptr, struct RecordVersion *record_version = nullptr, uint64_t &last_block_id=-1);
...
bool smallbank(const RepeatedPtrField<string> &keys, TransactionProposal::Type type, int execution_delay, bool expose_write,
               struct RecordVersion record_version= nullptr, Endorsement *endorsement = nullptr, long long last_block_id=-1);
#endif