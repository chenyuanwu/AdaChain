...
def run_agent(peer_config, client_config, peer_comm_stubs, num_episodes=1000, experiences_window=40):
    global dummy_X
    global throughput_Y
    global num_finished_peers
    ...
    early_execution_choice = [False, True]
    reorder_choice = [False, True]
    print("0")


    for blocksize in blocksizes:
        for early_execution in early_execution_choice:
            for reorder in reorder_choice:
                    peer_config['arch']['blocksize'] = blocksize # <--- add `id` value.
                    peer_config['arch']['early_execution'] = early_execution
                    peer_config['arch']['reorder'] = reorder
                    with open('../../config/peer_config.json', 'r+') as peer_config_f:
                        json.dump(peer_config, peer_config_f, indent=4)
                        print("1")

                    print("episode_cv:")
                    print(episode_cv)
                    print("finished peers:")
                    print(num_finished_peers)
                    ...
                    csv_writer.writerow([write_ratio, hot_key_ratio, trans_arrival_rate,
                                    execution_delay, blocksize, early_execution, reorder, throughput_current])
                    data_store.flush()
                    print("4")
...
if __name__ == '__main__':
    ...
    with open('../../config/client_config.json', 'r') as client_config_f:
        client_config = json.load(client_config_f) 
    ...
    try:
        ...
        run_agent(peer_config, client_config, stubs, 100, 100)
    finally:
